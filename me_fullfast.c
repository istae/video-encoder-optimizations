...
void SetupFastFullPelSearch (Macroblock *currMB, MEBlock *mv_block, int list)  // <--  reference frame parameter, list0 or 1
{
  ...
  if (apply_weights)
  {
    ...
    for (pos = 0; pos < max_pos; pos++)
    {
      ...
      {
#if (OPTIMIZE)
		  __m128i xmm_LineSadBlk0 = _mm_setzero_si128();
		  __m128i xmm_LineSadBlk1 = _mm_setzero_si128();
		  __m128i xmm_LineSadBlk2 = _mm_setzero_si128();
		  __m128i xmm_LineSadBlk3 = _mm_setzero_si128();
        for (y = 0; y < 4; y++)
        {
			weighted_pels[0] = iClip1(p_Vid->max_imgpel_value, ((weight_luma * *refptr++ + wp_luma_round) >> luma_log_weight_denom) + offset_luma);
			weighted_pels[1] = iClip1(p_Vid->max_imgpel_value, ((weight_luma * *refptr++ + wp_luma_round) >> luma_log_weight_denom) + offset_luma);
			weighted_pels[2] = iClip1(p_Vid->max_imgpel_value, ((weight_luma * *refptr++ + wp_luma_round) >> luma_log_weight_denom) + offset_luma);
			weighted_pels[3] = iClip1(p_Vid->max_imgpel_value, ((weight_luma * *refptr++ + wp_luma_round) >> luma_log_weight_denom) + offset_luma);
			
			__m128i xmm_weighted_pels = _mm_loadu_si128((__m128i*)weighted_pels);	
			__m128i xmm_srcptr = _mm_cvtsi64_si128(*(uint64_t *)srcptr);						
			xmm_srcptr = _mm_cvtepu16_epi32(xmm_srcptr);
			__m128i xmm_diff = _mm_sub_epi32(xmm_weighted_pels, xmm_srcptr);
			xmm_LineSadBlk0 = _mm_add_epi32(xmm_LineSadBlk0, _mm_abs_epi32(xmm_diff));
			srcptr += 4;
			...
			xmm_weighted_pels = _mm_loadu_si128((__m128i*)weighted_pels);
			xmm_srcptr = _mm_cvtsi64_si128(*(uint64_t *)srcptr);
			xmm_srcptr = _mm_cvtepu16_epi32(xmm_srcptr);
			xmm_diff = _mm_sub_epi32(xmm_weighted_pels, xmm_srcptr);
			xmm_LineSadBlk1 = _mm_add_epi32(xmm_LineSadBlk1, _mm_abs_epi32(xmm_diff));
			srcptr += 4;
			...
			xmm_srcptr = _mm_cvtepu16_epi32(xmm_srcptr);
			xmm_diff = _mm_sub_epi32(xmm_weighted_pels, xmm_srcptr);
			xmm_LineSadBlk2 = _mm_add_epi32(xmm_LineSadBlk2, _mm_abs_epi32(xmm_diff));
			srcptr += 4;
			...
			xmm_srcptr = _mm_cvtepu16_epi32(xmm_srcptr);
			xmm_diff = _mm_sub_epi32(xmm_weighted_pels, xmm_srcptr);
			xmm_LineSadBlk3 = _mm_add_epi32(xmm_LineSadBlk3, _mm_abs_epi32(xmm_diff));
			srcptr += 4;

		  refptr += p_Vid->padded_size_x - MB_BLOCK_SIZE;
		}

		__m128i shuffle = _mm_shuffle_epi32(xmm_LineSadBlk0, 14);	// xmm=[2,3,0,0]
		__m128i sum = _mm_add_epi32(xmm_LineSadBlk0, shuffle);
		shuffle = _mm_shuffle_epi32(sum, 1); // xmm=[1,0,0,0]
		sum = _mm_add_epi32(shuffle, sum);
		block_sad[bindex++][pos] = (distpel)_mm_cvtsi128_si32(sum);

		shuffle = _mm_shuffle_epi32(xmm_LineSadBlk1, 14);	// xmm=[2,3,0,0]
		sum = _mm_add_epi32(xmm_LineSadBlk1, shuffle);
		shuffle = _mm_shuffle_epi32(sum, 1); // xmm=[1,0,0,0]
		sum = _mm_add_epi32(shuffle, sum);
		block_sad[bindex++][pos] = (distpel)_mm_cvtsi128_si32(sum);

		shuffle = _mm_shuffle_epi32(xmm_LineSadBlk2, 14);	// xmm=[2,3,0,0]
		sum = _mm_add_epi32(xmm_LineSadBlk2, shuffle);
		shuffle = _mm_shuffle_epi32(sum, 1); // xmm=[1,0,0,0]
		sum = _mm_add_epi32(shuffle, sum);
		block_sad[bindex++][pos] = (distpel)_mm_cvtsi128_si32(sum);

		shuffle = _mm_shuffle_epi32(xmm_LineSadBlk3, 14);	// xmm=[2,3,0,0]
		sum = _mm_add_epi32(xmm_LineSadBlk3, shuffle);
		shuffle = _mm_shuffle_epi32(sum, 1); // xmm=[1,0,0,0]
		sum = _mm_add_epi32(shuffle, sum);
		block_sad[bindex++][pos] = (distpel)_mm_cvtsi128_si32(sum);
#else
		LineSadBlk1 = LineSadBlk2 = LineSadBlk3 = LineSadBlk0 = 0;
		for (y = 0; y < 4; y++)
		{
    ...
    for (pos = 0; pos < max_pos; pos++)
    {
      ...
      for (blky = 0; blky < 4; blky++)
      {
		...
		for (y = 0; y < 4; y++)
		{
			...
			__m128i xmm_refptr = _mm_cvtsi64_si128(*(uint64_t *)refptr);
			xmm_refptr = _mm_cvtepu16_epi32(xmm_refptr);
			__m128i xmm_srcptr = _mm_cvtsi64_si128(*(uint64_t *)srcptr);
			xmm_srcptr = _mm_cvtepu16_epi32(xmm_srcptr);
			__m128i xmm_diff = _mm_sub_epi32(xmm_refptr, xmm_srcptr);
			xmm_LineSadBlk0 = _mm_add_epi32(xmm_LineSadBlk0, _mm_abs_epi32(xmm_diff));
			srcptr += 4;
			refptr += 4;

			xmm_refptr = _mm_cvtsi64_si128(*(uint64_t *)refptr);
			xmm_refptr = _mm_cvtepu16_epi32(xmm_refptr);
			xmm_srcptr = _mm_cvtsi64_si128(*(uint64_t *)srcptr);
			xmm_srcptr = _mm_cvtepu16_epi32(xmm_srcptr);
			xmm_diff = _mm_sub_epi32(xmm_refptr, xmm_srcptr);
			xmm_LineSadBlk1 = _mm_add_epi32(xmm_LineSadBlk1, _mm_abs_epi32(xmm_diff));
			srcptr += 4;
			refptr += 4;
                              ...
                              )
{
  ...
  MotionVector* sqs = p_Vid->spiral_qpel_search;
  short off_y = offset->mv_y;
  short off_x = offset->mv_x;
  const __m128i xmm_offset = _mm_set_epi16(off_y, off_x, off_y, off_x, off_y, off_x, off_y, off_x);
  ...
  while (pos<max_pos)
  {
	  if (pos + 4 < max_pos) {
		  ...
		  __m128i xmm_mv = _mm_loadu_si128((void*)(sqs + pos));
		  __m128i xmm_mv_sum = _mm_add_epi16(xmm_mv, xmm_offset);

		  MotionVector* mv_sum = (MotionVector*)(&xmm_mv_sum);

		  for (int i = 0; i < 4;  i++, block_sad++, pos++) {
